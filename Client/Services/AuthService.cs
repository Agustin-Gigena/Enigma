...
public class AuthService
{
    ...
    public async Task<LoginResult> LoginAsync(string usuario, string contrasena)
    {
        try
        {
            var response = await _http.PostAsJsonAsync("auth/login", new { Usuario = usuario, Contrasena = contrasena });
            if (!response.IsSuccessStatusCode)
            {
                return new LoginResult(false, "Usuario o contraseña incorrectos.");
            }

            var datos = await response.Content.ReadFromJsonAsync<LoginResponse>(Json);
            if (datos is null)
            {
    ...
    {
        var json = await _js.InvokeAsync<string>("localStorage.getItem", UsuarioKey);
    ...
    {
        var json = await _js.InvokeAsync<string>("localStorage.getItem", InstitucionesKey);
        ...
        try
        {
            ...
            {
                using var request = new HttpRequestMessage(HttpMethod.Get, "auth/instituciones");
                request.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);
                var response = await _http.SendAsync(request);
                if (response.IsSuccessStatusCode)
                {
                    var instituciones = await response.Content.ReadFromJsonAsync<List<InstitucionDto>>(Json) ?? [];
    ...
    {
        var json = await _js.InvokeAsync<string>("localStorage.getItem", InstitucionActivaKey);
        return string.IsNullOrEmpty(json) ? null : JsonSerializer.Deserialize<InstitucionDto>(json, Json);
    }

    public async Task SetInstitucionActivaAsync(InstitucionDto institucion)
    {
        await _js.InvokeVoidAsync("localStorage.setItem", InstitucionActivaKey, JsonSerializer.Serialize(institucion, Json));
    }
