...
public class CurrentUserService : ICurrentUserService
{
  ...
  /// </summary>
  public static bool IsAuthenticated() => _current.Value?.Accessor.HttpContext?.User.Identity?.IsAuthenticated == true;

  /// <summary>
  /// Devuelve el ClaimsPrincipal del request actual (lectura viva vía el accessor) o null si no hay HttpContext. Nunca lanza.
  /// </summary>
  public static ClaimsPrincipal? GetClaimsPrincipal() => _current.Value?.Accessor.HttpContext?.User;
  ...
  /// </summary>
  internal static bool IsAuthRequired() => bool.TryParse(Environment.GetEnvironmentVariable("ENIGMA_AUTH_REQUIRED"), out bool b) && b;

  internal static void BeginScope(IHttpContextAccessor accessor, Func<int, Usuario?> resolver) => _current.Value = new CurrentUserScope(accessor, resolver);

  internal static void EndScope() => _current.Value = null;
